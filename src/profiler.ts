...
function calMin(bits:number, isSigned:number):number {
    if (bits <= 0) {
        return 0;
    }
    var min = 0;
...
function calMax(bits:number, isSigned:number):number {
    if (bits <= 0) {
        return 0;
    }
...
function convertValue(value:number, inputBitSize:number, isSigned:number, outputBitSize:number):number {
...
function readProfile(str:COLOR_PROFILE,totalSize?:boolean):object|number{
...
function isCOLOR_PROFILE(obj: any): obj is COLOR_PROFILE {
...
export function convertProfile(
    ...
    srcProfile:COLOR_PROFILE,
    dstProfile:COLOR_PROFILE,
    width?:number,
    height?:number,
    ):Buffer|Uint8Array{
    ...
    var srcSizeNeeded:number;
    var srcPixels = 0;
    var dstSizeNeeded:number;
    ...
    if(width != undefined && height != undefined){
        ...
        srcSizeNeeded = Math.ceil(srcPixels * (srcbpp as number / 8))
    } else {
        srcPixels = src.length / (srcbpp as number / 8)
        srcSizeNeeded = Math.ceil(srcPixels * (srcbpp as number / 8))
    }
    ...
    dstSizeNeeded = Math.ceil(srcPixels * (dstbpp as number / 8))

    var dstData = new Uint8Array(dstSizeNeeded)

    var src_read = readProfile(srcProfile) as PixelObjects[]
    var dst_read = readProfile(dstProfile) as PixelObjects[]
