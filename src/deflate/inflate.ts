...
class InflateState {
    ...
    constructor() {
        ...
        // TODO: may be {}
        this.head = <unknown>null as GZheader;           /* where to save gzip header information */
        ...
        this.window = <unknown>null as Uint8Array;         /* allocated sliding window, if needed */
        ...
        this.lencode = <unknown>null as Int32Array;          /* starting table for length/literal codes */
        this.distcode = <unknown>null as Int32Array;         /* starting table for distance codes */
        ...
        this.next = <unknown>null as number;              /* next available space in codes[] */
        ...
        */
        ...
        this.lendyn = <unknown>null as Int32Array;              /* dynamic table for length/literal codes (JS specific) */
        this.distdyn = <unknown>null as Int32Array;             /* dynamic table for distance codes (JS specific) */
...
const inflateStateCheck = (strm: ZStream): number => {
...
export function inflateResetKeep(strm: ZStream): number {
    ...
    state.flags = -1;
    state.dmax = 32768;
    state.head = <unknown>null as GZheader/*Z_NULL*/;
    state.hold = 0;
    state.bits = 0;
...
export function inflateReset(strm: ZStream): number {
...
export function inflateReset2(strm: ZStream, windowBits: number): number {
    let wrap: number;
    ...
    if (state.window !== null && state.wbits !== windowBits) {
        state.window = <unknown>null as Uint8Array;
    }
...
export function inflateInit2(strm: ZStream, windowBits: number): number {
    ...
    strm.state = state;
    state.strm = strm;
    state.window = <unknown>null as Uint8Array/*Z_NULL*/;
    state.mode = HEAD;     /* to pass state test in inflateReset2() */
    const ret = inflateReset2(strm, windowBits);
    if (ret !== Z_OK) {
        strm.state = <unknown>null as InflateState/*Z_NULL*/;
    }
    return ret;
};


export function inflateInit(strm: ZStream): number {
 ...
 */
const updatewindow = (strm: ZStream, src: Uint8Array, end: number, copy: number) => {
...
export function inflate(strm: ZStream, flush: number): number {
    ...
    state = strm.state as InflateState;
        ...
        switch (state.mode) {
            ...
            case EXLEN:
                if (state.flags & 0x0400) {
                    ...
                    state.head.extra = <unknown>null as Uint8Array/*Z_NULL*/;
                }
                state.mode = EXTRA;
            /* falls through */
            case EXTRA:
            ...
            case NAME:
                if (state.flags & 0x0800) {
                    ...
                    state.head.name = <unknown>null as string;
                }
                state.length = 0;
                state.mode = COMMENT;
            /* falls through */
            case COMMENT:
                if (state.flags & 0x1000) {
                    ...
                    state.head.comment = <unknown>null as string;
                }
                state.mode = HCRC;
            /* falls through */
            case HCRC:
...
export function inflateEnd(strm: ZStream): number {
    ...
    strm.state = <unknown>null as InflateState;
    return Z_OK;
};


export function inflateGetHeader(strm: ZStream, head: GZheader): number {
...
export function inflateSetDictionary(strm: ZStream, dictionary: Uint8Array): number {
