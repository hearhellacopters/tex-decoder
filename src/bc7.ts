...
type Bc7ModeInfo = {
    ...
    shared_pbits: usize,
    index_bits: Uint32Array,
}

const S_BP7_MODE_INFO: Bc7ModeInfo[] = [
    {
        num_subsets: 3,
        partition_bits: 4,
        rotation_bits: 0,
...
function expand_quantized(v: u8, bits: usize): u8 {
    let s = ((v) << (8 - bits)) & 0xFF;
    return s | (s >>> bits);
}
...
function decode_bc7_block(data: Uint8Array, outbuf: Uint32Array, block_num:number) {
    ...
    for (var yy = 0; yy < 4; yy++) {
        for (var xx = 0; xx < 4; xx++) {
            ...
            let aa =
                ((ep_a[subset_index] * faa + ep_a[subset_index + 1] * fab + 32) >> 6);
 ...
 */
export function decodeBC7(data: Uint8Array|Buffer, width: number, height:number): Uint8Array|Buffer {
