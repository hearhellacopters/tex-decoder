...
exports.decodeBC3 = exports.decodeDXT5 = exports.decodeDXT4 = exports.decodeDXT3 = exports.decodeBC2 = exports.decodeDXT2 = exports.decodeDXT1 = exports.decodeBC1 = void 0;
...
/*int */ height) {
    ...
    const asBuffer = isBuffer(input) ? true : false;
    ...
    for (var t = 0; t < bch; t++) {
        for (var s = 0; s < bcw; s++, offset += 8) {
            const r0 = new Int32Array(1), g0 = new Int32Array(1), b0 = new Int32Array(1), r1 = new Int32Array(1), g1 = new Int32Array(1), b1 = new Int32Array(1), q0 = new Int32Array([input[offset + 0] | input[offset + 1] << 8]), q1 = new Int32Array([input[offset + 2] | input[offset + 3] << 8]);
            Rgb565(q0, r0, g0, b0);
            Rgb565(q1, r1, g1, b1);
            ...
            const loc = offset + 4;
            var d = (((input[loc + 3] & 0xFF) << 24) | ((input[loc + 2] & 0xFF) << 16) | ((input[loc + 1] & 0xFF) << 8) | (input[loc] & 0xFF)) >>> 0;
...
function interpolateColorValues(firstVal, secondVal, isDxt1) {
    ...
    if (isDxt1 && firstVal <= secondVal) {
        colorValues.push(Math.round((firstColor[0] + secondColor[0]) / 2), Math.round((firstColor[1] + secondColor[1]) / 2), Math.round((firstColor[2] + secondColor[2]) / 2), 255, 0, 0, 0, 0);
    }
    else {
...
function decodeDXTn3(input, width, height, premultiplied) {
    ...
    check_size(width, height, 4, input);
    var rgba = isBuffer(input) ? Buffer.alloc(width * height * 4) : new Uint8Array(width * height * 4), height_4 = (height / 4) | 0, width_4 = (width / 4) | 0, offset = 0, alphaValues, alphaIndices, alphaIndex, alphaValue, multiplier, colorValues, colorIndices, colorIndex, pixelIndex, rgbaIndex, h, w, x, y;
...
function decodeDXTn2(input, width, height, premultiplied) {
    ...
    check_size(width, height, 4, input);
    var rgba = isBuffer(input) ? Buffer.alloc(width * height * 4) : new Uint8Array(width * height * 4), height_4 = (height / 4) | 0, width_4 = (width / 4) | 0, offset = 0, alphaValues, alphaValue, multiplier, colorValues, colorIndices, colorIndex, pixelIndex, rgbaIndex, h, w, x, y;
    for (h = 0; h < height_4; h++) {
        for (w = 0; w < width_4; w++) {
            alphaValues = [
                ((input[(offset + 6) + 1] << 8) | input[(offset + 6)]) & 0xFFFF,
