...
exports.decodeATC4 = exports.decodeATC8 = exports.decodeATC = exports.decodeBC5 = exports.decodeATI2 = exports.decodeBC4 = exports.decodeATI1 = void 0;
 ...
 */
function decodeATI1(data, width, height) {
...
function decodeBC4(data, width, height) {
    ...
    for (let by = 0; by < num_blocks_y; by++) {
        for (let bx = 0; bx < num_blocks_x; bx++) {
            decode_bc3_alpha(data.subarray(data_offset, data_offset + raw_block_size), buffer, 2);
 ...
 * @returns ```Uint8Array``` or ```Buffer``` as RGBA
 */
function decodeATI2(data, width, height) {
...
function decodeBC5(data, width, height) {
    ...
    for (let by = 0; by < num_blocks_y; by++) {
        for (let bx = 0; bx < num_blocks_x; bx++) {
            decode_bc3_alpha(data.subarray(data_offset, data_offset + raw_block_size), buffer, 2);
            decode_bc3_alpha(data.subarray(data_offset + 8, data_offset + 8 + raw_block_size), buffer, 1);
            copy_block_buffer(bx, by, width, height, BLOCK_WIDTH, BLOCK_HEIGHT, buffer, image);
            data_offset += raw_block_size;
        }
    }
 ...
 * @returns ```Uint8Array``` or ```Buffer``` as RGBA
 */
function decodeATC(data, width, height, Do8bitMode) {
    if (Do8bitMode) {
        return decodeATC8(data, width, height);
    }
    else {
        return decodeATC4(data, width, height);
    }
}
 ...
 * @returns ```Uint8Array``` or ```Buffer``` as RGBA
 */
function decodeATC8(data, width, height) {
    if (!isArrayOrBuffer(data)) {
        throw new Error(`Source data must be Uint8Array or Buffer`);
    }
    const image = new Uint32Array(width * height);
    ...
    for (let by = 0; by < num_blocks_y; by++) {
        for (let bx = 0; bx < num_blocks_x; bx++) {
            decode_atc_rgb4_block(data.subarray(data_offset + 8, data_offset + 8 + raw_block_size), buffer);
            decode_bc3_alpha(data.subarray(data_offset, data_offset + raw_block_size), buffer, 3);
            copy_block_buffer(bx, by, width, height, BLOCK_WIDTH, BLOCK_HEIGHT, buffer, image);
            data_offset += raw_block_size;
        }
    }
 ...
 * @returns ```Uint8Array``` or ```Buffer``` as RGBA
 */
function decodeATC4(data, width, height) {
    if (!isArrayOrBuffer(data)) {
        throw new Error(`Source data must be Uint8Array or Buffer`);
    }
    const image = new Uint32Array(width * height);
    ...
    for (let by = 0; by < num_blocks_y; by++) {
        for (let bx = 0; bx < num_blocks_x; bx++) {
            decode_atc_rgb4_block(data.subarray(data_offset, data_offset + raw_block_size), buffer);
            copy_block_buffer(bx, by, width, height, BLOCK_WIDTH, BLOCK_HEIGHT, buffer, image);
            data_offset += raw_block_size;
        }
    }
